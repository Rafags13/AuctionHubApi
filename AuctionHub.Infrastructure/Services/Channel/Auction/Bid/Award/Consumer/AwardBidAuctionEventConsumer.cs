...
namespace AuctionHub.Infrastructure.Services.Channel.Auction.Bid.Award.Consumer
{
    internal sealed class AwardBidAuctionEventConsumer(
        ...
        ChannelReader<ChannelDTO<AwardBidAuctionEvent>> channel,
            ...
            try
            {
                ...
                await foreach (var @event in channel.ReadAllAsync(stoppingToken))
                {
                    try
                    {
                        ...
                        var outBidId = await bidRepository.GetOutBidIdAsync(message.AuctionId, stoppingToken);
                        ...
                        if ((outBidId.HasValue &&
                            ...
                            !await auctionRepository.UpdateCurrentPriceAsync(
                               new RequestUpdateAuctionCurrentPriceDTO(message.AuctionId, message.Amount), stoppingToken
                        ))
                        {
                        ...
                        if (outBidId.HasValue)
                        {
                            var auctionOutBidInformations = await auctionRepository.GetOutBidAsync(message.AuctionId, stoppingToken);
                        ...
                        logger.LogInformation("Auction {AuctionId} approved the bid of value {BidId} at {Time}", message.AuctionId, message.Amount, DateTime.UtcNow);
                        ...
                        activity?.SetTag("auction.amoun", message.Amount);
                    }
                    catch (Exception ex)
                    {
